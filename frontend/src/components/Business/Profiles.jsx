...
const Profiles = () => {
  ...
  const [profileData, setProfileData] = useState({ name: "", contact: "", address: "" });
  ...
  useEffect(() => {
    const fetchComparison = async () => {
      ...
      try {
        ...
        if (selectedJob) query.set("job_title", selectedJob);
        const response = await fetch(`${PYTHON_API_URL}/api/business/candidate-comparison?${query.toString()}`);
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Failed to load candidate comparison.");
        setComparison(data);
      } catch (err) {
  ...
  useEffect(() => {
    if (mainTab === "profile" && auth.email) {
      setProfileLoading(true);
      fetch(`${NODE_API_URL}/api/business/profile?email=${auth.email}`)
        .then(res => res.json())
        .then(data => setProfileData({ name: data.name || "", contact: data.contact || "", address: data.address || "" }))
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      const res = await fetch(`${NODE_API_URL}/api/business/profile`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ currentEmail: auth.email, ...profileData })
      });
  ...
  const selectedSession = useMemo(() => {
    ...
    return comparison.sessions.find((session) => session.session_id === selectedSessionId) || comparison.sessions[0] || null;
  }, [comparison.sessions, selectedSessionId]);
  const parsedResume = useMemo(() => safeParseResume(selectedSession?.candidate_resume), [selectedSession?.candidate_resume]);
  const selectedResponse = selectedSession?.responses?.[0] || null;

  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      <div className="bg-orange-500 p-6 text-white">
        <h1 className="text-2xl font-bold">Business Profile & Candidate Rankings</h1>
        <div className="mt-4 flex gap-4">
          <button
            onClick={() => setMainTab("rankings")}
            className={`px-4 py-2 font-semibold rounded-md ${mainTab === "rankings" ? "bg-white text-orange-600" : "bg-orange-600 text-white hover:bg-orange-700"}`}
          >
            Candidate Rankings
          </button>
          <button
            onClick={() => setMainTab("profile")}
            className={`px-4 py-2 font-semibold rounded-md ${mainTab === "profile" ? "bg-white text-orange-600" : "bg-orange-600 text-white hover:bg-orange-700"}`}
          >
            Company Profile
          </button>
        </div>
      ...
      <div className="container mx-auto p-6 space-y-6">
        {mainTab === "profile" && (
          <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
            <h2 className="text-xl font-semibold text-gray-800 mb-6">Update Company Profile</h2>
            {profileLoading ? (
              ...
              <form onSubmit={handleProfileSave} className="space-y-4">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="bg-orange-600 text-white px-6 py-2 rounded shadow hover:bg-orange-700 disabled:bg-orange-300"
                  >
